...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('register_claim', function (Blueprint $table) {
            $table->id('id_register_claim');
            $table->string('no_polis');
            $table->foreignId('id_register_survey')
            ->constrained()
            ->references('id_register_survey')
            ->on('register_survey')
            ->onUpdate('cascade')
            ->onDelete('no action');
    ...
    {
        Schema::dropIfExists('register_claim');
    }
};
